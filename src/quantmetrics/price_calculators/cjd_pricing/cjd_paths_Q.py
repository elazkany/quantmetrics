#src\quantmetrics\price_calculators\cjd_pricing\cjd_paths_Q.py
...
class CJDSimulatePathsQ:
    ...
    def simulate(self, num_timesteps: int, num_paths: int, seed: int) -> Dict[str, np.ndarray]:
        ...
        .. math::
            ...
            s_{i+1, j} \\approx s_{i,j} + (r-q - \\lambda^\\mathbb{Q} \\kappa^\\mathbb{Q}) s_{i,j} \\Delta t + \\sigma s_{i,j} (W_{i+1, j} - W_{i,j}) + \\gamma s_{i,j} (N_{i+1,j} -N_{i,j}),

        with :math:`\\Delta t = t_{i+1} - t_i`, for any :math:`i=1,2,\\cdots , m, \\ s_0 = S(t_0)=S_0`, :math:`j = 1,2,\\cdots, N`, :math:`W_{i+1, j} - W_{i,j} \\sim \\mathcal{N}(0, \Delta t)` and :math:`N_{i+1,j} -N_{i,j} \sim \\mathcal{Poi}(\\lambda^\\mathbb{Q} \\Delta t)`. 

        The CJD process has as exact solution in the time interval :math:`[t_i, t_{i+1}]`,

        .. math::

            S(t_{i+1})=S(t_i)\\exp\\left\{(r-q-\\frac{\\sigma^2}{2}- \\lambda^\\mathbb{Q} \\kappa^\\mathbb{Q})\\Delta t + \\sigma [W(t_{i+1}) - W(t_i)] + \\gamma [N(t_{i+1}) - N(t_i)] \\right\}
        ...
        .. math::
            ...
            \\lambda^\\mathbb{Q} = \\lambda \\quad \\text{and} \\quad \\kappa^\\mathbb{Q} = \\kappa = e^\\gamma - 1

        If ``emm = "Esscher"`` then

        .. math::

            \\lambda^\\mathbb{Q} = \\lambda \\exp \\left(\\theta \\gamma + \\psi \\gamma^2 \\right) \\quad \\text{and} \\quad \\kappa^\\mathbb{Q} = \\kappa = e^\\gamma - 1
        ...
        >>> paths = CJDSimulatePathsQ(cjd, option).simulate(num_timesteps=200, num_paths=10000,seed=42)
        ...
        """
        return self._cjd_simulate_paths_Q(num_timesteps, num_paths, seed)

    def _cjd_simulate_paths_Q(self, num_timesteps: int, num_paths: int, seed: int) -> Dict[str, np.ndarray]:
        """
        Generate paths for the constant jump-diffusion (CJD) model.
       ...
       """
        ...
        kappa = np.exp(gamma) - 1

        if emm == "mean-correcting":
            lambda_Q = lambda_
        else:
            # solve the martinglae equation for the risk premium value
            theta = RiskPremium(self.model, self.option).calculate()
            lambda_Q = (r - mu - theta * sigma**2 + lambda_ * kappa) / kappa
        ...
        for i in range(0, num_timesteps):
            ...
            S_exact[:,i + 1] = S_exact[:,i] * np.exp( (r - q -0.5 *sigma*sigma - lambda_Q * kappa)*dt + sigma * (W[:,i + 1] - W[:,i])+ gamma * ZPois[:, i])
                    
            S_euler[:,i + 1] = S_euler[:,i] + (r-q - lambda_Q * kappa) * S_euler[:,i] * dt + sigma * S_euler[:,i] * (W[:,i + 1] - W[:,i]) +(np.exp(gamma)-1)* S_euler[:,i] * ZPois[:,i]
        ...
        # lambda_Q * kappa * dt is the compensator of the jump process gamma N_t
        kappa = np.exp(gamma) - 1
        ...
        incr_exact = np.exp((r - q - 0.5 * sigma**2 - lambda_Q * kappa) * dt + sigma * dW + gamma * ZPois)
        ...
        incr_euler = 1 + (r - q - lambda_Q * kappa) * dt + sigma * dW + (np.exp(gamma)-1)* ZPois
