...
class OptionChainPreprocessor:
    ...
    def _calculate_days_to_maturity(self):
        ...
        """
        self.dataframe['days_to_maturity'] = (self.dataframe['exdate'] - self.dataframe['date']).dt.days

    def _calculate_mid_price(self):
        """
        Calculate the mid price of the option.
        """
        self.dataframe['mid_price'] = (self.dataframe['best_bid'] + self.dataframe['best_offer']) / 2
    ...
    def _adjust_strike_price(self, divisor : int =1000):
        ...
        """
        self.dataframe['strike_price'] = self.dataframe['strike_price'].astype(float) / divisor
    ...
    def _subset_open_interest(self, open_interest_range : tuple =(0, float('inf'))):
        ...
        self.dataframe = self.dataframe[(self.dataframe['open_interest'] >= min_oi) & (self.dataframe['open_interest'] <= max_oi)]
    ...
    def _subset_volume(self, volume_range : tuple =(0, float('inf'))):
        ...
        self.dataframe = self.dataframe[(self.dataframe['volume'] >= min_volume) & (self.dataframe['volume'] <= max_volume)]
    ...
    def _download_underlying_price(
        ...
        try:
             asset_data = yf.download(underlying_ticker, start=start_date, end=end_date, progress=False)[[price_col]]
             asset_data.columns = [underlying_ticker]
             self.dataframe = self.dataframe.merge(asset_data, left_on='date', right_index=True, how='left')
    ...
    def _download_interest_rate(
        ...
        try:
                ...
                self.dataframe = self.dataframe.merge(interest_rate_data, left_on='date', right_index=True, how='left')
        except Exception as e:
    ...
    def _download_vix(
        ...
        """
        try:
                asset_data = yf.download('^VIX', start=start_date, end=end_date, progress=False)[[price_col]]
                asset_data.columns = ['^VIX']
                self.dataframe = self.dataframe.merge(asset_data, left_on='date', right_index=True, how='left')
        except Exception as e:
                print(f"Error fetching VIX data: {e}")

    def _subset_by_moneyness(
              self, 
        ...
        if self.underlying_ticker is None:
             raise ValueError("Underlying ticker not found in the dataframe. Please ensure the underlying asset price has been downloaded by editing the download_underlying_price input in the preprocess method")
        ...
        for (date, days_to_maturity), group in grouped:
                ...
                min_strike = np.min(np.floor(S0 * np.arange(lower_rate, upper_rate + step_size, step_size)))
                max_strike = np.max(np.floor(S0 * np.arange(lower_rate, upper_rate + step_size, step_size)))
        
                subset = group[(group['strike_price'] >= min_strike) & (group['strike_price'] <= max_strike)]
